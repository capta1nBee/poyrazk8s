...
@RestController
@RequestMapping("/api/clusters/{clusterUid}/certificatesigningrequests")
@RequiredArgsConstructor
public class CertificateSigningRequestActionController {
    ...
    private boolean hasPermission(String clusterUid, String resourceName, String action) {
        ...
        if (user.getIsSuperadmin() != null && user.getIsSuperadmin())
            return true;
        return authorizationService.hasPermission(user, clusterUid, null, "CertificateSigningRequest", resourceName,
                action);
    }
    ...
    public ApiResponse<String> updateYaml(@PathVariable String clusterUid, @PathVariable String name,
        ...
        return ApiResponse.success("CertificateSigningRequest updated successfully");
    }
    ...
    public ApiResponse<String> delete(@PathVariable String clusterUid, @PathVariable String name) {
        ...
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        service.deleteCertificateSigningRequest(clusterId, name);
        return ApiResponse.success("CertificateSigningRequest deleted successfully");
    }
}
