...
@RestController
@RequestMapping("/api/clusters/{clusterUid}/clusterrolebindings")
@RequiredArgsConstructor
public class ClusterRoleBindingActionController {
    ...
    private boolean hasPermission(String clusterUid, String resourceName, String action) {
        ...
        if (user.getIsSuperadmin() != null && user.getIsSuperadmin())
            return true;
        return authorizationService.hasPermission(user, clusterUid, null, "ClusterRoleBinding", resourceName, action);
    }
    ...
    public ApiResponse<String> updateYaml(@PathVariable String clusterUid, @PathVariable String name,
        ...
        return ApiResponse.success("ClusterRoleBinding updated successfully");
    }
    ...
    public ApiResponse<String> delete(@PathVariable String clusterUid, @PathVariable String name) {
        ...
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        service.deleteClusterRoleBinding(clusterId, name);
        return ApiResponse.success("ClusterRoleBinding deleted successfully");
    }
}
