...
@RestController
@RequestMapping("/api/clusters/{clusterUid}/customresourcedefinitions")
@RequiredArgsConstructor
public class CustomResourceDefinitionActionController {
    ...
    private boolean hasPermission(String clusterUid, String resourceName, String action) {
        ...
        if (user.getIsSuperadmin() != null && user.getIsSuperadmin())
            return true;
        return authorizationService.hasPermission(user, clusterUid, null, "CustomResourceDefinition", resourceName,
                action);
    }
    ...
    public ApiResponse<String> updateYaml(@PathVariable String clusterUid, @PathVariable String name,
        ...
        return ApiResponse.success("CustomResourceDefinition updated successfully");
    }
    ...
    public ApiResponse<String> delete(@PathVariable String clusterUid, @PathVariable String name) {
        ...
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        service.deleteCustomResourceDefinition(clusterId, name);
        return ApiResponse.success("CustomResourceDefinition deleted successfully");
    }
}
