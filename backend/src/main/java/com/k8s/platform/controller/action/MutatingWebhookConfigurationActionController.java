...
@RestController
@RequestMapping("/api/clusters/{clusterUid}/mutatingwebhookconfigurations")
@RequiredArgsConstructor
public class MutatingWebhookConfigurationActionController {
    ...
    private boolean hasPermission(String clusterUid, String resourceName, String action) {
        ...
        if (user.getIsSuperadmin() != null && user.getIsSuperadmin())
            return true;
        return authorizationService.hasPermission(user, clusterUid, null, "MutatingWebhookConfiguration",
                resourceName, action);
    }
    ...
    public ApiResponse<String> updateYaml(@PathVariable String clusterUid, @PathVariable String name,
        ...
        return ApiResponse.success("MutatingWebhookConfiguration updated successfully");
    }
    ...
    public ApiResponse<String> delete(@PathVariable String clusterUid, @PathVariable String name) {
        ...
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        service.deleteMutatingWebhookConfiguration(clusterId, name);
        return ApiResponse.success("MutatingWebhookConfiguration deleted successfully");
    }
}
