...
@RestController
@RequestMapping("/api/clusters/{clusterUid}/priorityclasses")
@RequiredArgsConstructor
public class PriorityClassActionController {
    ...
    private boolean hasPermission(String clusterUid, String resourceName, String action) {
        ...
        if (user.getIsSuperadmin() != null && user.getIsSuperadmin())
            return true;
        return authorizationService.hasPermission(user, clusterUid, null, "PriorityClass", resourceName, action);
    }
    ...
    public ApiResponse<String> updateYaml(@PathVariable String clusterUid, @PathVariable String name,
        ...
        return ApiResponse.success("PriorityClass updated successfully");
    }
    ...
    public ApiResponse<String> delete(@PathVariable String clusterUid, @PathVariable String name) {
        ...
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        service.deletePriorityClass(clusterId, name);
        return ApiResponse.success("PriorityClass deleted successfully");
    }
}
