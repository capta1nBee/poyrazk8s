...
@RestController
@RequestMapping("/api/clusters/{clusterUid}/prioritylevelconfigurations")
@RequiredArgsConstructor
public class PriorityLevelConfigurationActionController {
    ...
    private boolean hasPermission(String clusterUid, String resourceName, String action) {
        ...
        if (user.getIsSuperadmin() != null && user.getIsSuperadmin())
            return true;
        return authorizationService.hasPermission(user, clusterUid, null, "PriorityLevelConfiguration", resourceName,
                action);
    }
    ...
    public ApiResponse<String> updateYaml(@PathVariable String clusterUid, @PathVariable String name,
        ...
        return ApiResponse.success("PriorityLevelConfiguration updated successfully");
    }
    ...
    public ApiResponse<String> delete(@PathVariable String clusterUid, @PathVariable String name) {
        ...
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        service.deletePriorityLevelConfiguration(clusterId, name);
        return ApiResponse.success("PriorityLevelConfiguration deleted successfully");
    }
}
