...
@RestController
@RequestMapping("/api/clusters/{clusterUid}/validatingwebhookconfigurations")
@RequiredArgsConstructor
public class ValidatingWebhookConfigurationActionController {
    ...
    private boolean hasPermission(String clusterUid, String resourceName, String action) {
        ...
        if (user.getIsSuperadmin() != null && user.getIsSuperadmin())
            return true;
        return authorizationService.hasPermission(user, clusterUid, null, "ValidatingWebhookConfiguration",
                resourceName, action);
    }
    ...
    public ApiResponse<String> updateYaml(@PathVariable String clusterUid, @PathVariable String name,
        ...
        return ApiResponse.success("ValidatingWebhookConfiguration updated successfully");
    }
    ...
    public ApiResponse<String> delete(@PathVariable String clusterUid, @PathVariable String name) {
        ...
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        service.deleteValidatingWebhookConfiguration(clusterId, name);
        return ApiResponse.success("ValidatingWebhookConfiguration deleted successfully");
    }
}
