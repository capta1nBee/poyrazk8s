...
@RestController
@RequestMapping("/api/clusters/{clusterUid}/volumeattachments")
@RequiredArgsConstructor
public class VolumeAttachmentActionController {
    ...
    private boolean hasPermission(String clusterUid, String resourceName, String action) {
        ...
        if (user.getIsSuperadmin() != null && user.getIsSuperadmin())
            return true;
        return authorizationService.hasPermission(user, clusterUid, null, "VolumeAttachment", resourceName, action);
    }
    ...
    public ApiResponse<String> updateYaml(@PathVariable String clusterUid, @PathVariable String name,
        ...
        return ApiResponse.success("VolumeAttachment updated successfully");
    }
    ...
    public ApiResponse<String> delete(@PathVariable String clusterUid, @PathVariable String name) {
        ...
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        service.deleteVolumeAttachment(clusterId, name);
        return ApiResponse.success("VolumeAttachment deleted successfully");
    }
}
