...
public class ValidatingAdmissionPolicyController {
    ...
    @GetMapping("/validatingadmissionpolicies")
    public List<ValidatingAdmissionPolicy> listValidatingAdmissionPolicies(@PathVariable String clusterUid) {
        authHelper.checkPermissionOrThrow(clusterUid, "*", "ValidatingAdmissionPolicy", "*", "view");
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        return service.listValidatingAdmissionPolicies(clusterId);
    }

    @GetMapping("/validatingadmissionpolicies/{name}")
    public ValidatingAdmissionPolicy getValidatingAdmissionPolicy(@PathVariable String clusterUid,
            @PathVariable String name) {
        authHelper.checkPermissionOrThrow(clusterUid, "*", "ValidatingAdmissionPolicy", name, "view");
        Long clusterId = k8sClientService.getClusterIdByUid(clusterUid);
        return service.getValidatingAdmissionPolicy(clusterId, name);
    }
}
