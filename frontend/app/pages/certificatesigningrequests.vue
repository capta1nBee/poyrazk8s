...
import type { CertificateSigningRequest } from '~/types/certificatesigningrequest'

const clusterStore = useClusterStore()
const k8s = useKubernetes()
const toast = useToast()
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('CertificateSigningRequest')

const resources = ref<CertificateSigningRequest[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<CertificateSigningRequest>('CertificateSigningRequest', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch CertificateSigningRequests',
...
<template>
  <UDashboardPanel id="certificatesigningrequests">
    <template #header>
      <UDashboardNavbar title="Certificate Signing Requests">
        <template #leading>
      ...
      <UDashboardToolbar v-if="hasPermission('view')">
        <template #left>
          ...
          <UBadge color="neutral" variant="subtle">{{ resources.length }} requests</UBadge>
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Certificate Signing Requests.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="CertificateSigningRequest" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
