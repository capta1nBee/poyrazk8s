...
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('ClusterRoleBinding')

const resources = ref<ClusterRoleBinding[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<ClusterRoleBinding>('ClusterRoleBinding', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch ClusterRoleBindings',
...
<template>
  <UDashboardPanel id="clusterrolebindings">
    <template #header>
      <UDashboardNavbar title="Cluster Role Bindings">
        <template #leading>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Cluster Role Bindings.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="ClusterRoleBinding" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
