...
import type { CSIDriver } from '~/types/csidriver'

const clusterStore = useClusterStore()
const k8s = useKubernetes()
const toast = useToast()
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('CSIDriver')

const resources = ref<CSIDriver[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<CSIDriver>('CSIDriver', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch CSIDrivers',
...
<template>
  <UDashboardPanel id="csidrivers">
    <template #header>
      <UDashboardNavbar title="CSI Drivers">
        <template #leading>
      ...
      <UDashboardToolbar v-if="hasPermission('view')">
        <template #left>
          ...
          <UBadge color="neutral" variant="subtle">{{ resources.length }} drivers</UBadge>
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view CSI Drivers.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="CSIDriver" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
