...
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('Role')

const resources = ref<Role[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<Role>('Role', undefined, includeDeleted.value)
    ...
    <template #header>
      ...
      <UDashboardToolbar v-if="hasPermission('view')">
        <template #left>
          ...
          <UBadge color="neutral" variant="subtle">{{ resources.length }} roles</UBadge>
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Roles.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="Role" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
