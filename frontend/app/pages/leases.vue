...
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('Lease')
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<any>('Lease', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch Leases',
    ...
    <template #header>
      ...
      <UDashboardToolbar>
        <template #left>
          ...
          <UBadge color="neutral" variant="subtle">{{ resources.length }} leases</UBadge>
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
        <template #age-data="{ row }">
          <span class="text-sm text-gray-600 dark:text-gray-400">{{ useTimeAgo(row.metadata?.creationTimestamp).value }}</span>
        </template>
        <template #actions-data="{ row }">
          <ResourceActionMenu :resource="row" kind="Lease" @refresh="fetchData" />
        </template>
      </LegacyTable>
      </template>
      <div v-else class="flex flex-col items-center justify-center h-64 text-center">
        ...
        <p class="text-sm text-gray-500 mt-1">You do not have permission to view Leases.</p>
      </div>
    </template>
  </UDashboardPanel>
</template>
