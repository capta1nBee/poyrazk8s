...
import type { MutatingWebhookConfiguration } from '~/types/mutatingwebhookconfiguration'

const clusterStore = useClusterStore()
const k8s = useKubernetes()
const toast = useToast()
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('MutatingWebhookConfiguration')

const resources = ref<MutatingWebhookConfiguration[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<MutatingWebhookConfiguration>('MutatingWebhookConfiguration', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch MutatingWebhookConfigurations',
...
<template>
  <UDashboardPanel id="mutatingwebhookconfigurations">
    <template #header>
      <UDashboardNavbar title="Mutating Webhooks">
        <template #leading>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Mutating Webhooks.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="MutatingWebhookConfiguration" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
