...
import type { PriorityClass } from '~/types/priorityclass'

const clusterStore = useClusterStore()
const k8s = useKubernetes()
const toast = useToast()
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('PriorityClass')

const resources = ref<PriorityClass[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<PriorityClass>('PriorityClass', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch PriorityClasses',
...
<template>
  <UDashboardPanel id="priorityclasses">
    <template #header>
      <UDashboardNavbar title="Priority Classes">
        <template #leading>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Priority Classes.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="PriorityClass" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
