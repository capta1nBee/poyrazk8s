...
import type { PriorityLevelConfiguration } from '~/types/prioritylevelconfiguration'

const clusterStore = useClusterStore()
const k8s = useKubernetes()
const toast = useToast()
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('PriorityLevelConfiguration')

const resources = ref<PriorityLevelConfiguration[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<PriorityLevelConfiguration>('PriorityLevelConfiguration', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch PriorityLevelConfigurations',
...
<template>
  <UDashboardPanel id="prioritylevelconfigurations">
    <template #header>
      <UDashboardNavbar title="Priority Level Configs">
        <template #leading>
      ...
      <UDashboardToolbar v-if="hasPermission('view')">
        <template #left>
          ...
          <UBadge color="neutral" variant="subtle">{{ resources.length }} configs</UBadge>
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Priority Level Configs.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="PriorityLevelConfiguration" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
