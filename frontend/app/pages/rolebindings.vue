...
import type { RoleBinding } from '~/types/rolebinding'

const clusterStore = useClusterStore()
const k8s = useKubernetes()
const toast = useToast()
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('RoleBinding')

const resources = ref<RoleBinding[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<RoleBinding>('RoleBinding', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch RoleBindings',
...
<template>
  <UDashboardPanel id="rolebindings">
    <template #header>
      <UDashboardNavbar title="Role Bindings">
        <template #leading>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Role Bindings.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="RoleBinding" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
