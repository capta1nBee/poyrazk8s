...
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('ServiceAccount')

const resources = ref<ServiceAccount[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<ServiceAccount>('ServiceAccount', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch ServiceAccounts',
...
<template>
  <UDashboardPanel id="serviceaccounts">
    <template #header>
      ...
      <UDashboardToolbar v-if="hasPermission('view')">
        <template #left>
          ...
          <UBadge color="neutral" variant="subtle">{{ resources.length }} accounts</UBadge>
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Service Accounts.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="ServiceAccount" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
