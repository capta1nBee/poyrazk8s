...
import type { ValidatingAdmissionPolicyBinding } from '~/types/validatingadmissionpolicybinding'

const clusterStore = useClusterStore()
const k8s = useKubernetes()
const toast = useToast()
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('ValidatingAdmissionPolicyBinding')

const resources = ref<ValidatingAdmissionPolicyBinding[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<ValidatingAdmissionPolicyBinding>('ValidatingAdmissionPolicyBinding', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch ValidatingAdmissionPolicyBindings',
...
<template>
  <UDashboardPanel id="validatingadmissionpolicybindings">
    <template #header>
      <UDashboardNavbar title="Validating Admission Policy Bindings">
        <template #leading>
      ...
      <UDashboardToolbar v-if="hasPermission('view')">
        <template #left>
          ...
          <UBadge color="neutral" variant="subtle">{{ resources.length }} bindings</UBadge>
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Validating Admission Policy Bindings.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="ValidatingAdmissionPolicyBinding" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
