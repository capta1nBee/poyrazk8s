...
import type { ValidatingWebhookConfiguration } from '~/types/validatingwebhookconfiguration'

const clusterStore = useClusterStore()
const k8s = useKubernetes()
const toast = useToast()
const { hasPermission, loading: permissionsLoading, filterByName } = usePagePermissions('ValidatingWebhookConfiguration')

const resources = ref<ValidatingWebhookConfiguration[]>([])
...
const fetchData = async () => {
  ...
  loading.value = true
  try {
    const result = await k8s.fetchResources<ValidatingWebhookConfiguration>('ValidatingWebhookConfiguration', undefined, includeDeleted.value)
    resources.value = result
  } catch (error: any) {
    toast.add({
      title: 'Failed to fetch ValidatingWebhookConfigurations',
...
<template>
  <UDashboardPanel id="validatingwebhookconfigurations">
    <template #header>
      <UDashboardNavbar title="Validating Webhooks">
        <template #leading>
      ...
      <UDashboardToolbar v-if="hasPermission('view')">
        <template #left>
          ...
          <UBadge color="neutral" variant="subtle">{{ resources.length }} webhooks</UBadge>
        </template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      ...
      <template v-else-if="!hasPermission('view')">
        <div class="flex flex-col items-center justify-center h-full space-y-4">
          ...
          <p class="text-gray-500">You do not have permission to view Validating Webhooks.</p>
        </div>
      </template>
      <template v-else>
        <LegacyTable :rows="filteredResources" :columns="columns" :loading="loading" class="w-full">
          ...
          <template #actions-data="{ row }">
            <ResourceActionMenu :resource="row" kind="ValidatingWebhookConfiguration" @refresh="fetchData" />
          </template>
        </LegacyTable>
      </template>
    </template>
